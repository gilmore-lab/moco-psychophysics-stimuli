classdef ObjSet < handle
    ...
    properties (SetObservable)
        ...
        deviceNumber = GetKeyboardIndices; % Input Device Settings
        scanList = []; % List of allowable response keys
        t % Timer object
    end
    ...
    methods
        ...
        function start_fcn(obj) % start_fcn method for timer
            ...
            fprintf('ObjSet Line 327: start_fcn(obj) called\n');
            
            %RestrictKeysForKbCheck(obj.start_key);
            %enabledKeys = [obj.start_key];
            ...
            if obj.sys.display.dual
                ...
                [~,keyCode,~] = KbStrokeWait(obj.deviceNumber);
                if find(keyCode) == obj.start_key(end)
                    fprintf('ObjSet: line 336: escape key hit during blank screen\n');
                    obj.pres.esc_flag = 1;
                    stop(obj.t)
                    ListenChar(0);
                    return;
                ...
                [~,keyCode,~] = KbStrokeWait(obj.deviceNumber);
                if find(keyCode) == obj.start_key(end)
                    fprintf('ObjSet: line 352: escape key hit during fixation screen\n');
                    obj.pres.esc_flag = 1;
                    stop(obj.t)
                    ListenChar(0);
                    return;
                ...
                % Blank Screen
                % what does this line do, it is on line 332 above?
                obj.pres.flip_fun(obj.sys.display.w);
            else
                ...
                [~,keyCode,~] = KbStrokeWait(obj.deviceNumber);
                if find(keyCode) == obj.start_key(end)
                    fprintf('ObjSet: line 369: escape key hit during ? screen\n');                
                    obj.pres.esc_flag = 1;
                    stop(obj.t)
                    ListenChar(0);
                    return;
                ...
                [~,keyCode,~] = KbStrokeWait(obj.deviceNumber);
                if find(keyCode) == obj.start_key(end)
                    fprintf('ObjSet: line 384: escape key hit during ????? screen\n');
                    obj.pres.esc_flag = 1;
                    stop(obj.t)
                    ListenChar(0);
                end
        ...
        function timer_fcn(obj,b,t) % timer_fcn method for timer
            ...
            % RestrictKeysForKbCheck(obj.timer_key); **************** rog 2016-0205
            %enabledKeys = [obj.timer_key];
            %scanList = zeros(1,256);
            %scanList(enabledKeys) = 1;
            %obj.scanList = scanList;
            
               %fprintf('ObjSet: line 404:Restrict keys for keyboard check during dot display\n');
        ...
        function stop_fcn(obj) % stop_fcn method for timer
            fprintf('ObjSet Line 445: stop_fcn(obj) called\n');
    ...
    methods (Static)
        ...
        function exp = ExpSet(sys,subjstr)
            ...
            exp.block = 5; % Number of blocks: 5 for study, 1 for presentation purposes
            ...
            exp.coherence = [.15 .30 .45 .60]; % Coherence conditions - child beginning 5-10-2016
%             exp.coherence = [.2 .4 .6 .8]; % Coherence conditions - child 1-2016 thru 5-9-2016
%             exp.coherence = [.2 .8]; %adult Coherence Conditions
%             exp.coherence = [.7 .8 .9]; % Practice Coherence conditions
            exp.v = [ 2 ]; % Dot speed (deg/sec) [2 or 8]
            ...
            exp.block_count = 13; % Block counter
...
%             end
        ...
        function pres = PresSet(sys)
            ...
            %if obj.deviceNumber == GetKeyboardIndices
            pres.txt = 'Motion coherence discrimination task.\n\n\n  Press spacebar to begin.';
            %elseif obj.deviceNumber == GetGamepadIndices
            %    pres.txt = 'Motion coherence discrimination task.\n\n\n  Press 6 to begin.';   
            %end
